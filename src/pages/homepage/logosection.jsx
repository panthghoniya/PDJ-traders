      ...
      <div data-animate="fade-up" data-delay="300" className="relative flex overflow-x-hidden group">
        <div className="flex animate-marquee whitespace-nowrap py-4">
          {marqueeItems.map((country, idx) => (
            <div
              key={`${country.id}-${idx}`}
              className={`mx-3 md:mx-4 flex-shrink-0 w-32 h-32 md:w-40 md:h-40 rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.04)] flex items-center justify-center ${country.bg} transition-all duration-300 hover:-translate-y-2 cursor-pointer border border-brand-dark/5`}
            >
